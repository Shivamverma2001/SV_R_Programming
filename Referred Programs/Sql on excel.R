...
library("readxl")
#getwd()
Q2=read_excel("C:/Users/tanim/OneDrive/Documents/Q2.xls")
sqldf("select * from Q2 limit 10")# display 10 rows
sqldf("select Profit as 'P',
      Quantity from Q2 limit 10")
#filtering the data
...
sqldf("select City,Profit,State from Q2 order by 
City, State desc  limit 20")
sqldf("select City,State,Profit from Q2 order by 
      City desc,State  limit 20")
#like
sqldf("select city,profit from Q2 where City like
'H%' limit 5")
sqldf("select city,profit from Q2 where City like
      'He%' ")
sqldf("select city,profit from Q2 where City like 
...
sqldf("select sum(Profit) as 'Total_Profit' from 
      Q2 ")
sqldf("select min(Profit) as 'Min_Profit'from Q2 ")
sqldf("select max(Profit) as 'Max_Profit' from Q2 ")
sqldf("select count(Profit) as 'Number_of_records' 
      from Q2 ")
...
b=sqldf("select Segment, profit from Q2 order by
        Profit")
head(b)
#group by
sqldf("Select sum(Sales) from Q2 ")
sqldf("Select region,sum(Sales)as Total_Sale from Q2 
where Region in ('Central','West') group by Region")
sqldf("Select region,sum(Sales) from Q2 group 
      by Region")
sqldf("select sum(profit),region from Q2 group
      by region ")
sqldf("select sum(sales), category from Q2 
      group by category")
sqldf("select state,sum(profit) from Q2 group
      by state ")
sqldf("select region,sum(sales),sum(profit),
sum(sales)-sum(profit) as cost from Q2 
      group by region ")
