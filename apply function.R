#1. apply function array apply works on both
x=matrix(c(1,2,3,4),nrow = 3,ncol = 4,byrow = TRUE)
x
y=apply(x,1,sum)#1 means row
y
z=apply(x,2,sum)#2 means column
z
colSums(x)   
rowSums(x)

#2. lapply work on list list apply
...
sapply(List, length)
#sapply vs lapply : sapply returns a vector, matrix, array
#instead of a list. Consider the given example
sapply(c(3,5,7),exp)
lapply(c(3,5,7),exp)
#if you set the simplify argument of the sapply function to FALSE you will get the same output as the lapply function
...
id<-result$id
marks=result$marks
section<-factor(result$section,labels=c("Section A","Section B"))
section
...
vec2<-c(2,4,6,8)
vec3<-c(3,6,9,12)
mapply(function(v1,v2,v3)v1*v2*v3,vec1,vec2,vec3)
#another example
